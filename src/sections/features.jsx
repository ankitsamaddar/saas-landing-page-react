    ...
    <section>
      <Element name="features">
        <div className={'container'}>
          <div
            className={
              'relative flex md:flex-wrap flex-nowrap border-2 border-s3 rounded-7xl md:overflow-hidden max-md:flex-col feature-after md:g7 max-md:border-none max-md:gap-3'
            }
          >
            {features.map(({ id, icon, title, caption, text, button }) => (
              <div
                key={id}
                className={
                  'relative z-2 md:px-10 px-5 md:pb-10 pb-5 flex-50 max-md:g7 max-md:border-2 max-md:border-s3 max-md:rounded-3xl max-md:flex-320'
                }
              >
            ...
            <ul className="relative flex justify-around flex-grow px-[5%] border-2 border-s3 rounded-7xl max-md:hidden">
              ...
              {details.map(({ id, icon, title }) => (
                <li key={id} className="relative pt-16 px-4 pb-14">
                  ...
                  <h3 className="relative z-2 max-w-36 mx-auto my0 base-small text-center uppercase">
                    {title}
                  </h3>
                </li>
              ))}
